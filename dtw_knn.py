...
def fit(X, y, model_name="dtwknn"):
    ...
    clf = GridSearchCV(
        KNeighborsTimeSeriesClassifier(
            metric="dtw", metric_params={"global_constraint": "itakura", "itakura_max_slope": 1.}, n_jobs=-1
        ),
        parameters,
        cv=2,
        n_jobs=-1,
...
if __name__ == "__main__":
    ...
    for data_type in data_types:
        y_arr.append(np.load(f"dtw_data_npy/y_{data_type}_1min_entropy.npy")[:1000])
        x_arr.append(np.load(f"dtw_data_npy/x_{data_type}_1min_entropy.npy")[:1000])
